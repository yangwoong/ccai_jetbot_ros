...
# `cmake --install` with no prefix defaults to /usr/local - which lives in the
# container's own writable layer, NOT the bind-mounted repo, so it is wiped
# every time the container is recreated (host_docker_run.sh does `docker rm
# -f` + `docker run`). That's exactly what happened after the first
# successful build here: the container was recreated to pick up
# CCAI_ENABLE_DEPTH_NAV/the USB mount, and realsense2_camera_node then failed
# to even start ("librealsense2.so.2.58: cannot open shared object file") -
# librealsense2 itself was gone. Installing into deps/librealsense/install
# instead keeps it on the bind-mounted, persistent side.
REALSENSE_INSTALL_PREFIX="${DEPS_DIR}/librealsense/install"
LIBRESENSE_ENV_FILE="${DEPS_DIR}/librealsense/librealsense_env.sh"
mkdir -p "${REALSENSE_INSTALL_PREFIX}"
cat > "${LIBRESENSE_ENV_FILE}" <<EOF2
# Generated by install_realsense_d435i.sh - source this before building or
# running anything that links against librealsense2 (realsense2_camera,
# a manual 'ros2 launch realsense2_camera ...', etc). container_run_patrol.sh /
# container_build.sh already source it automatically when present.
export LD_LIBRARY_PATH="${REALSENSE_INSTALL_PREFIX}/lib:\${LD_LIBRARY_PATH:-}"
export CMAKE_PREFIX_PATH="${REALSENSE_INSTALL_PREFIX}:\${CMAKE_PREFIX_PATH:-}"
export PKG_CONFIG_PATH="${REALSENSE_INSTALL_PREFIX}/lib/pkgconfig:\${PKG_CONFIG_PATH:-}"
EOF2
set +u
source "${LIBRESENSE_ENV_FILE}"
set -u
...
if pkg-config --exists realsense2 2>/dev/null; then
  ...
  cmake -S "${DEPS_DIR}/librealsense" -B "${DEPS_DIR}/librealsense/build" \
    -DCMAKE_INSTALL_PREFIX="${REALSENSE_INSTALL_PREFIX}" \
