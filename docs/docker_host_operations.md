...
Jetson CSI 카메라는 Argus를 사용하므로, USB `/dev/video0`와 분리해서 실행합니다. CSI 모드는 항상 `/tmp:/tmp`를 마운트하고 `--ipc host`, `--runtime nvidia`를 사용합니다(`host_docker_run.sh` 기본값이며 별도 옵션이 없습니다). 이는 `nvargus-daemon` 재시작 후 `/tmp/argus_socket` 단일 마운트가 stale 상태가 되는 문제를 피하기 위한 것입니다.
...
CSI가 Docker 안에서 `Failed to create CaptureSession`으로 매번 실패하면, 대부분 `nvargus-daemon`의 systemd 설정 문제입니다(`docs/hardware_jetbot.md`의 "CSI가 안 나올 때 가장 먼저 확인할 것" 절 참고 — `Environment="enableCamInfiniteTimeout=1"`을 제거하면 해결되는 경우가 많습니다). 그 다음에도 Docker 안에서만 CSI가 안 되면(호스트에서는 되는데) `docs/hardware_jetbot.md`의 호스트 MJPEG 브리지 우회 절차를 참고하세요 — 정상 상황에서는 필요 없는 예외 경로입니다.
...
`systemd/ccai-jetbot.service`는 Jetson이 부팅되면 Docker가 뜬 뒤 `host_docker_run.sh`를 안전 모드 없이(`CCAI_SAFE_START=0`) 자동 실행하도록 등록되어 있습니다.
...
유닛 파일은 `/home/roboat/work/ros2_ws/ccai_jetbot_ros`를 기준으로 되어 있습니다. 경로나 사용자가 다르면 `WorkingDirectory`, `EnvironmentFile`, `ExecStart`, `User`를 맞춰서 수정하세요. `CCAI_SAFE_START`나 개별 `CCAI_ENABLE_*` 값을 다르게 하고 싶으면 유닛 파일의 `Environment=` 줄에 추가하거나 `.env`에 넣습니다.

```bash
sudo systemctl status ccai-jetbot.service
sudo systemctl restart ccai-jetbot.service
sudo systemctl stop ccai-jetbot.service
```
