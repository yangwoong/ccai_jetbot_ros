...
`compute_steering_twist`(커버리지 탐색이 쓰는 컨트롤러)는 "목표 방향으로 정렬됐는가"를 `visual_odom_node`의 yaw로 판단하는데, 실기에서 로봇이 몇 초씩 회전해도 yaw가 거의 안 바뀌는 게 확인됐습니다(빠른 회전 중 ORB 추적이 깨지거나, 카메라-로봇 축 매핑의 스케일 자체가 안 맞을 가능성 - 둘 다 아직 미해결). 그래서 이번 방-단위 탐색은 **yaw를 아예 참조하지 않고**, `angular_speed` 파라미터와 목표 각도로 계산한 시간만큼만 회전합니다(`(2π/explore_room_scan_headings) / angular_speed`). 장점은 이 알고리즘 자체가 오도메트리 버그와 완전히 무관해진다는 것이고, 단점은 실제 회전량이 바닥 마찰/배터리 전압 등에 따라 매번 조금씩 다를 수 있어 여러 방을 거칠수록 방향 오차가 누적된다는 것입니다(정직한 한계 - 아래 참고).
...
- **전체 파이프라인 실기 미검증**: 이 알고리즘 자체를 아직 로봇에서 한 번도 돌려보지 못했습니다. 각 단계 전환/타이밍/VLM 파싱이 실제로 맞물려 돌아가는지 확인이 필요합니다.
- **회전 각도 캘리브레이션 없음**: `angular_speed`로 계산한 시간이 실제 45도에 얼마나 정확한지 실측 안 됨 - 바퀴 미끄러짐/배터리 전압에 따라 오차가 있을 수 있고, 여러 방을 거칠수록 누적됩니다.
- **`explore_room_scan_advance_seconds`(기본 4.0초) 미보정**: 출입구를 통과하는 데 필요한 실제 거리/시간을 이 로봇/이 환경 기준으로 튜닝하지 않았습니다.
- **VLM 응답 형식 신뢰도**: `DOOR:YES`/`DOOR:NO` 프리픽스로 응답해달라고 프롬프트에 요청했지만, LLM이 항상 정확히 그 형식을 지킬 거란 보장은 없습니다 - 느슨한 키워드 폴백(`출입구`/`통로`/`문`)도 같이 넣어뒀지만 오탐/누락 가능성이 있습니다.
