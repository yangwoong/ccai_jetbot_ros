...
- **5차 수정 실기 미검증**: 0.3m 정지거리, DOOR/FLOOR 구분, 무한 재탐색 전부 이 절 작성 시점에 아직 로봇에서 돌려보지 못했습니다 - 특히 0.3m가 실제로 안전한지(로봇이 18cm급으로 작다지만 급정지 거리/속도에 따라 재조정 필요할 수 있음) 실기 확인이 최우선입니다.
